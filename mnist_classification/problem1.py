...
def compute_a(z):
    ...
    new_z = np.clip(z - np.max(z), -700, None) # clipping to prevent underflow
    exp_vals = []

    # calculating numerator of softmax activation function
    for val in new_z:
        exp_vals.append(np.exp(val))
    
    A = np.sum(exp_vals) # denominator of softmax activation function
    A_vals = []

    # populating A_vals with components of vector a
    for val in exp_vals:
        A_vals.append(val / A)

    # making A_vals into a vector
    a = np.array(A_vals)
...
def compute_L(a,y):
    ...
    # returns large loss if log(0) needs to be computed
    if a[y] == 0.0:
        return 1e6
...
def compute_dL_da(a, y):
    ...
    # dL_da = np.zeros_like(a)
    # dL_da[y] = -1 / a[y] if a[y] != 0 else -1e6
...
def compute_da_dz(a):
    ...
    # diag is used when i = j
    # outer is used when i != j
